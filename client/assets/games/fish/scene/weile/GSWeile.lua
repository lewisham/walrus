...
-- 等待场景创建
function M:waitForCreateScene()
    wls.WaitForFuncResult(function() return self:find("SCGameClient"):get("create_scene") end)
    self:createRoot()
end
...
function M:initUI()
    ...
    self:find("SCSend"):startHeartBeatTimer()
    self:initSkill()
    for i = 1, 4 do
...
function M:waitForReady()
    wls.SendMsg("sendReady")
    wls.WaitForFuncResult(function() return self:find("SCRecv"):get("start_process") end)
end
